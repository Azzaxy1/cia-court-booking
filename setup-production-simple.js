...
async function setupProduction() {
  try {
    ...
    const deactivateResponse = await fetch(`${baseUrl}/api/system-protection`, {
      method: "POST",
    ...
    const initResponse = await fetch(`${baseUrl}/api/system-protection`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action: "initialize",
    ...
    const statusResponse = await fetch(`${baseUrl}/api/system-protection`);
